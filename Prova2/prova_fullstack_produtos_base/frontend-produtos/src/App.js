...
function App() {
  ...
  async function cadastrarProduto(evento) {
    ...
    try {
      ...
      setMensagem("Erro de conexão ao cadastrar produto.");
    }
  }
  ...
  async function buscarProdutoPorId() {
    ...
    try {
      ...
      if (resposta.ok) {
        ...
        setMensagem(`Produto ID ${idBusca} encontrado!`);
      } else if (resposta.status === 404) {
        setProdutoEncontrado(null);
        setMensagem(`Produto com ID ${idBusca} não encontrado.`);
      } else {
        setMensagem("Erro ao buscar produto.");
      }
    } catch (erro) {
      setMensagem("Erro de conexão ao buscar produto.");
      setProdutoEncontrado(null);
    }
  }
  ...
  async function atualizarProduto() {
    if (!produto.id) {
      setMensagem("Selecione um produto para editar primeiro!");
      return;
    }
  ...
  async function excluirProduto(id) {
    ...
    try {
      ...
      if (resposta.ok) {
        ...
        if (produtoEncontrado && produtoEncontrado.id === id) {
          setProdutoEncontrado(null);
        }
      } else if (resposta.status === 404) {
        setMensagem("Produto não encontrado para exclusão.");
      } else {
        setMensagem("Erro ao excluir produto.");
      }
    } catch (erro) {
      setMensagem("Erro de conexão ao excluir produto.");
    }
  }
